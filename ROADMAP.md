...
最後更新：2026-06-06（完成 15 漸層產生器：左控制台＋右大舞台即時預覽；linear／radial／conic 三種漸層，linear/conic 可調角度（滑桿＋8 個快捷鈕）、radial 可選 circle/ellipse；棋盤底漸層條可「點空白新增色標／拖曳把手調位置」，新增色標自動內插接續當前漸層色；選取色標以滑桿調位置、色票＋HEX 雙向同步、可刪除（至少留 2 個）；6 組預設（暮色／柑橘／海洋／硃紅手冊／墨紙／極光）一鍵整組替換；CSS 依色標位置排序輸出、複製埋 track('use')；自寫零相依 hex↔rgb 換算與線性內插，維持 CSP script-src 'self'；入口頁徽章 is-live＋搜尋計數改 15、package.json 補編譯映射）
...
- [ ] 16 圖片格式轉換：PNG ↔ WebP ↔ JPEG ↔ AVIF（與 01 共用部分邏輯）
