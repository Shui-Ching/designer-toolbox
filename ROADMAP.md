...
- [ ] 17 等等吃喝什麼（決策轉盤）：使用者自訂選項清單（新增／刪除，可命名，至少 2 項才能轉）；點「轉！」觸發轉盤旋轉動畫，緩速停止後高亮結果；轉盤以 Canvas 或純 CSS 繪製扇形，依選項數均分角度、自動配色；選項可清空並重設；入口頁徽章 is-live＋搜尋計數改 17、package.json 補編譯映射；data-category="fun"（若有此類別）
- [ ] 18 PDF 壓縮：純前端拖放上傳 PDF，以 pdf-lib（本機 bundle，維持 CSP script-src 'self'）重新打包，去除 metadata、壓縮嵌入圖片品質（有損／無損兩模式），顯示壓縮前後大小對比，下載壓縮後 PDF；對超大 PDF 加檔案大小提示；下載埋 track('use')；入口頁徽章 is-live＋搜尋計數改 18、package.json 補編譯映射
...
### 回饋管道（全站功能，待規劃）
- [ ] 在入口頁 footer 加「意見回饋」入口，區分三類：
  - **Bug 回報**：使用工具時碰到的問題
  - **優化建議**：現有功能的改善意見
  - **功能許願池**：想要的新工具或新功能
- 實作方向（選一）：
  - A. 連結到 GitHub Issues（貼上 bug／feature request template）—— 零後端、最省力
  - B. 各類別對應一個 Google Form 連結 —— 視覺整潔但需維護表單
  - C. 工具頁內嵌 `mailto:` 帶預填主旨 —— 純靜態但開信箱體驗差
- 待確認：採用哪種管道後再動手實作
